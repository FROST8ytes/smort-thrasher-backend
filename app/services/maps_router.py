...
class MapsRouter:
    ...
    def generate_multi_stop_url(self, coordinates_json: str, origin: str) -> str:
        try:
            ...
            points = [
                f"{coord['latitude']},{coord['longitude']}" for coord in coordinates]
    ...
    def sorting_waypoints(self, coordinates_json: str, origin: str,
        ...
        try:
            ...
            all_points = [
                origin] + [f"{c['latitude']},{c['longitude']}" for c in coordinates]
            ...
            for _ in range(1, n):
                next_point = min(
                    ((i, distance_matrix[current][i])
                     for i in range(1, n) if not visited[i]),
                    key=lambda x: x[1],
                    default=(None, None)
                )[0]
            ...
            sorted_coordinates = [coordinates[i - 1]
                                  for i in order]  # skip origin
